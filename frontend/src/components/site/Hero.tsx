...
export function Hero() {
  ...
  return (
    <section ref={root} className="section-shell mesh-grid relative pt-36 md:pt-44 pb-24 md:pb-32 overflow-hidden" id="top">
      <div
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 grid lg:grid-cols-2 gap-12 items-center relative">
        <div className="relative z-10">
          <motion.p className="hero-sub eyebrow mb-6" whileInView={{ opacity: [0.5, 1], y: [8, 0] }} viewport={{ once: true }}>
            AI assistant for students, fresh graduates, and active job seekers
          </motion.p>
          <h1 className="hero-title headline-mega text-balance">
            Your job search,<br />handled end to end.
          </h1>
          <p className="hero-sub mt-6 max-w-xl text-base text-muted-foreground leading-relaxed">
            HIREOS learns your skills, education, experience, and preferences, then builds your resume,
            drafts outreach emails, finds matching openings, and manages follow-ups on your behalf.
          </p>

          <div className="hero-cta mt-10 flex flex-wrap items-center gap-4">
            <motion.a href="#how" className="btn-accent" whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }}>
              Get started <ArrowRight className="w-4 h-4" />
            </motion.a>
            <a href="#how" className="text-sm font-medium text-muted-foreground hover:text-foreground transition">
              Learn how it works →
            </a>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {["Builds your resume", "Finds matching jobs", "Sends and follows up"].map((chip) => (
              <span key={chip} className="frost rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em]">
                {chip}
              </span>
            ))}
          </div>
